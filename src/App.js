...
const App = () => {
  ...
  useEffect(() =>{
    axios.get('https://telugu-skillhub-own-music-api-default-rtdb.firebaseio.com/music-api/-MdCU2vPsoSsulYD2akz.json').then(
      res => {
        {/* By default we have set search value be null, if we are not searched we are getting entire data
             otherwise we are getting the user input search Data  if the singer name in API is eqauls to user input search variable*/}
